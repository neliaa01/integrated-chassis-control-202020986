...
err = yawRateRef - yawRate;
    ...
    if ~isfield(ctrlState,'filteredDerr'), ctrlState.filteredDerr = 0; end
    
    % 속도에 따른 조향 게인 스케일링 정밀 튜닝
    speedGain = 12 / max(vx, 6); 
    speedGain = min(max(speedGain, 0.52), 0.95); 
    
    if abs(yawRateRef) < 1e-3
        err = 0;
        ctrlState.intError = 0;
    else
        ctrlState.intError = ctrlState.intError + 1.0 * err * dt;
        ctrlState.intError = min(max(ctrlState.intError, -CTRL.LAT.intMax), CTRL.LAT.intMax);
    end
    
    tau = 0.20; 
    raw_derr = (err - ctrlState.prevError) / max(dt, 1e-3);
    alpha = dt / (tau + dt);
    derr = alpha * raw_derr + (1 - alpha) * ctrlState.filteredDerr;
    
    if isnan(derr) || isinf(derr) || abs(derr) > 200
        derr = 0;
    end
    
    ctrlState.filteredDerr = derr;
    ctrlState.prevError = err;
    
    % 선회 복원력과 궤적 안전성의 표준 게인 스케일링
    deltaSteer = speedGain * (CTRL.LAT.Kp * 1.2 * err + CTRL.LAT.Ki * ctrlState.intError + CTRL.LAT.Kd * 0.18 * derr);
    
    beta_th = 0.60 * LIM.MAX_SLIP_ANGLE; 
    yawMoment = 0;
    
    % 차량 스핀 방지를 위한 ESC 능동 개입 루프
    if abs(slipAngle) > beta_th
        K_steer_beta = 0.08; 
        deltaSteer = deltaSteer + K_steer_beta * sign(slipAngle) * (abs(slipAngle) - beta_th);
        
        Kbeta = 9500; 
        yawMoment = -Kbeta * sign(slipAngle) * (abs(slipAngle) - beta_th) * max(vx/10, 1.0);
    end
    
    deltaSteer = min(max(deltaSteer, -LIM.MAX_STEER_ANGLE), LIM.MAX_STEER_ANGLE);
    
    deltaAdd.steerAngle = deltaSteer;
