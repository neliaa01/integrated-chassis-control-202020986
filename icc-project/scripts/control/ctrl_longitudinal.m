...
err = vxRef - vx;
    ...
    % 타 시나리오(A4 등) 오염을 방지하기 위한 순수 오차 감지형 셋팅
    if (vxRef < vx - 1.0)
        % 타이어 락업과 감속 거리의 최적 타협점
        if ax < -4.5
            Fx = -18000; 
        else
            Fx = -24000; 
        ...
        % 일반 주행 및 선회 가속 구간 (A4 시나리오 오염 원천 차단)
        ...
        Fx = 22000 * (CTRL.LON.Kp * err + CTRL.LON.Ki * ctrlState.intError);
        
        maxDeltaF = LIM.MAX_JERK * 1500 * dt; 
        dF = Fx - ctrlState.prevForce;
        dF = min(max(dF, -maxDeltaF), maxDeltaF);
